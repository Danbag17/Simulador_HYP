...
export function CronogramaEstados() {
    ...
    for (let i = 0; i < historialSeguro.length; i++) {
        ...
        if (i === 0 || estado !== historialSeguro[i - 1].state) {
            transiciones.push({ estado, t: historialSeguro[i].t });
        }
    }
    ...
    return (
        <div className="bg-white rounded-lg border border-gray-200 shadow-sm px-3 py-2 flex items-center gap-3">
            <span className="text-[9px] font-bold text-gray-300 tracking-widest uppercase whitespace-nowrap">Estados</span>

            {/* Barra de estados en línea */}
                        ...
                        <div key={key} className="flex items-center gap-1 flex-shrink-0">
                            <span
                                style={{
                                    ...
                                    boxShadow: esActual ? `0 0 6px ${cfg.color}99` : 'none',
                                    opacity: fuePasado || esActual ? 1 : 0.3,
                                    outline: esActual ? `2px solid ${cfg.color}` : 'none',
                                    outlineOffset: '1px',
                                }}
                                className="px-1.5 py-0.5 rounded text-[9px] font-bold text-white transition-all"
                            >
                                {cfg.label}
                            </span>
            ...
            {/* Última transición registrada */}
            {transiciones.length > 0 && (
                <span className="ml-auto text-[9px] font-mono text-gray-400 whitespace-nowrap flex-shrink-0">
                    {new Date(transiciones[transiciones.length - 1].t).toLocaleTimeString()}
                </span>
            )}
        </div>
    );
